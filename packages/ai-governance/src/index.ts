...
import { handleaigovernanceAssessment } from "./tools/ai-governance-assessment.js";
import { handleaigovernanceScan } from "./tools/ai-governance-scan.js";

const server = new McpServer({
  name: "csoai-ai-governance-mcp",
  version: "1.0.0"
});
...
server.tool("ai_governance_assessment", "Run AI Governance assessment against target", AssessmentShape, handleaigovernanceAssessment);
server.tool("ai_governance_scan", "Run AI Governance scan on target", ScanShape, handleaigovernanceScan);

async function main() {
  const transport = new StdioServerTransport();
  await server.connect(transport);
  console.error("csoai-ai-governance-mcp running on stdio");
}
