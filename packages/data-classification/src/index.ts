...
import { handledataclassificationAssessment } from "./tools/data-classification-assessment.js";
import { handledataclassificationScan } from "./tools/data-classification-scan.js";

const server = new McpServer({
  name: "csoai-data-classification-mcp",
  version: "1.0.0"
});
...
server.tool("data_classification_assessment", "Run Data Classification assessment against target", AssessmentShape, handledataclassificationAssessment);
server.tool("data_classification_scan", "Run Data Classification scan on target", ScanShape, handledataclassificationScan);

async function main() {
  const transport = new StdioServerTransport();
  await server.connect(transport);
  console.error("csoai-data-classification-mcp running on stdio");
}
