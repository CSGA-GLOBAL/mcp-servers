...
import { handlepolicyengineAssessment } from "./tools/policy-engine-assessment.js";
import { handlepolicyengineScan } from "./tools/policy-engine-scan.js";

const server = new McpServer({
  name: "csoai-policy-engine-mcp",
  version: "1.0.0"
});
...
server.tool("policy_engine_assessment", "Run Policy Engine assessment against target", AssessmentShape, handlepolicyengineAssessment);
server.tool("policy_engine_scan", "Run Policy Engine scan on target", ScanShape, handlepolicyengineScan);

async function main() {
  const transport = new StdioServerTransport();
  await server.connect(transport);
  console.error("csoai-policy-engine-mcp running on stdio");
}
