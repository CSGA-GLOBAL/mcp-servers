...
export function handleAvLiabilityFramework(
  systemName: string,
  liabilityModel: string,
  incidentType: string,
  insuranceCoverage: string,
  jurisdiction: string
): handleAvLiabilityFrameworkResult {
  const jurLower = jurisdiction.toLowerCase();
  const fnLower = liabilityModel.toLowerCase();

  let riskClassification = "Standard autonomous vehicles ai AI use";
  ...
  if (regulations.length === 0) {
    ...
    regulations.push("Industry-specific regulations for autonomous vehicles ai");
  }
