...
import { handlecomplianceauditAssessment } from "./tools/compliance-audit-assessment.js";
import { handlecomplianceauditScan } from "./tools/compliance-audit-scan.js";

const server = new McpServer({
  name: "csoai-compliance-audit-mcp",
  version: "1.0.0"
});
...
server.tool("compliance_audit_assessment", "Run Compliance Audit assessment against target", AssessmentShape, handlecomplianceauditAssessment);
server.tool("compliance_audit_scan", "Run Compliance Audit scan on target", ScanShape, handlecomplianceauditScan);

async function main() {
  const transport = new StdioServerTransport();
  await server.connect(transport);
  console.error("csoai-compliance-audit-mcp running on stdio");
}
