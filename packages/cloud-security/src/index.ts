...
import { handlecloudsecurityAssessment } from "./tools/cloud-security-assessment.js";
import { handlecloudsecurityScan } from "./tools/cloud-security-scan.js";

const server = new McpServer({
  name: "csoai-cloud-security-mcp",
  version: "1.0.0"
});
...
server.tool("cloud_security_assessment", "Run Cloud Security assessment against target", AssessmentShape, handlecloudsecurityAssessment);
server.tool("cloud_security_scan", "Run Cloud Security scan on target", ScanShape, handlecloudsecurityScan);

async function main() {
  const transport = new StdioServerTransport();
  await server.connect(transport);
  console.error("csoai-cloud-security-mcp running on stdio");
}
