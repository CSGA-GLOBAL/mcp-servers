...
import { handleincidentresponseAssessment } from "./tools/incident-response-assessment.js";
import { handleincidentresponseScan } from "./tools/incident-response-scan.js";

const server = new McpServer({
  name: "csoai-incident-response-mcp",
  version: "1.0.0"
});
...
server.tool("incident_response_assessment", "Run Incident Response assessment against target", AssessmentShape, handleincidentresponseAssessment);
server.tool("incident_response_scan", "Run Incident Response scan on target", ScanShape, handleincidentresponseScan);

async function main() {
  const transport = new StdioServerTransport();
  await server.connect(transport);
  console.error("csoai-incident-response-mcp running on stdio");
}
